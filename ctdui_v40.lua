...
-- Build: PASS33-AUTORETRY-CLICK-CASCADE-V40
...
if type(session.endClickOriginalHandleV40) ~= "function" then
    session.endClickOriginalHandleV40 = session.recorder.handleEndAction
end
local originalHandle = session.endClickOriginalHandleV40
...
local function logOffscreen(button)
    ...
    print("[XOMA V40] Restart ignored off-screen | " .. geometryText(position, size, viewport))
end
...
local function stopReplayForRealEnd()
    ...
    if type(session.stopReplayForEndScreen) == "function" then
        ...
        if not ok then warn("[XOMA V40] Replay stop hook failed | " .. tostring(err)) end
    end
end
...
local function tryNextClickMethod(endFrame, container, restartButton, returnButton)
    ...
    if control.methodIndex > #methods then
        ...
        print("[XOMA V40] Auto Retry click methods exhausted | cycling again")
    ...
    if not ok then
        print(string.format(
            "[XOMA V40] Auto Retry method skipped | %s | %s",
            method.name,
            tostring(detail)
        ))
    ...
    print(string.format(
        "[XOMA V40] Auto Retry direct click | method=%s | %s | %s",
        method.name,
        tostring(detail),
...
session.endClickWatcherSerialV40 = (tonumber(session.endClickWatcherSerialV40) or 0) + 1
local watcherSerial = session.endClickWatcherSerialV40

task.spawn(function()
    while session.alive and session.endClickWatcherSerialV40 == watcherSerial do
        task.wait(0.12)
...
session.endClickBuild = "PASS33-AUTORETRY-CLICK-CASCADE-V40"
print("[XOMA V40] Auto Retry click-cascade watcher installed")
