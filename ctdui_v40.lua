...
do
    local source = [===[-- XOMA Auto Retry authoritative visible-button click
-- Build: PASS30-MONOLITH-AUTORESTART-LIVE-BUTTON-V40
...
session.recorder.handleEndAction = function(...)
    ...
    if not result then
        if os.clock() - control.lastDiag >= 3 then
            ...
            print(string.format(
                "[XOMA V40] Restart visible but result missing | PlayerGameResult=%s | Rewards.Result=%s",
                tostring(playerText),
                tostring(rewardText)
            ))
        end
    ...
    if alreadyAccepted then
        ...
        if control.key ~= acceptedKey then
            ...
            print("[XOMA V40] Auto Retry already accepted | " .. tostring(whyAccepted))
        end
        return
    end
    ...
    if control.key ~= key then
        ...
        control.busy = false
        print("[XOMA V40] Live Restart detected | result=" .. result .. " | source=" .. source)
    end
    ...
    print(string.format(
        "[XOMA V40] Auto Retry VIM click %d | sent=%s | result=%s via %s | before=%s | %s",
        control.attempts,
        tostring(ok),
        result,
        source,
...
session.endClickBuild = "PASS30-MONOLITH-AUTORESTART-LIVE-BUTTON-V40"
print("[XOMA V40] Auto Restart live-button VIM installed")
