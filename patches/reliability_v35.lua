...
-- Build: PASS25-AUTORETRY-RECONNECT-RECORDER-V35
-- Auto Retry is authoritative, network disconnects rejoin the same place, and
-- recorder output is normalized to the V35 cache-busted bootstrap.
...
then
    error("XOMA V35 reliability patch: CTDIG/XOMA session unavailable")
end
...
local BOOTSTRAP_URL = "https://raw.githubusercontent.com/evilxoma/xomahub/refs/heads/main/ctdui_v35.lua"
...
local function upgradeSavedMacro()
    ...
    if not wrote then
        warn("[XOMA V35] macro bootstrap update failed: " .. tostring(err))
        return false
    end

    print("[XOMA V35] Saved macro bootstrap upgraded -> ctdui_v35.lua")
    return true
end

session.upgradeSavedMacroBootstrapV35 = upgradeSavedMacro
forceAutoRetry()
upgradeSavedMacro()
...
session.reconnectV35 = session.reconnectV35 or {
    active = false,
    attempts = 0,
    reason = nil,
}
local reconnect = session.reconnectV35
...
local function startReconnect(reason)
    ...
    warn("[XOMA V35] Network disconnect detected | rejoin armed | " .. reconnect.reason)

    task.spawn(function()
        task.wait(1)
        while session.alive and reconnect.active do
            reconnect.attempts = reconnect.attempts + 1
            print("[XOMA V35] Rejoin attempt " .. tostring(reconnect.attempts))
            local ok, err = pcall(function()
                TeleportService:Teleport(game.PlaceId, player)
            end)
            if not ok then
                warn("[XOMA V35] Rejoin request failed: " .. tostring(err))
            end
...
session.reliabilityBuild = "PASS25-AUTORETRY-RECONNECT-RECORDER-V35"
print("[XOMA V35] Auto Retry forced ON | reconnect installed | recorder -> V35")
return session.XOMA
