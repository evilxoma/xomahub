...
-- Build: PASS26-AUTORETRY-RECONNECT-RECORDER-V36
-- Auto Retry is authoritative, network disconnects rejoin the same place, and
-- recorder output is normalized to the V36 cache-busted bootstrap.
...
then
    error("XOMA V36 reliability patch: CTDIG/XOMA session unavailable")
end
...
local BOOTSTRAP_URL = "https://raw.githubusercontent.com/evilxoma/xomahub/refs/heads/main/ctdui_v36.lua"
...
local function upgradeSavedMacro()
    ...
    if not wrote then
        warn("[XOMA V36] macro bootstrap update failed: " .. tostring(err))
        return false
    end

    print("[XOMA V36] Saved macro bootstrap upgraded -> ctdui_v36.lua")
    return true
end

session.upgradeSavedMacroBootstrapV36 = upgradeSavedMacro
forceAutoRetry()
upgradeSavedMacro()

if session.reliabilityMonitorV36Installed ~= true then
    session.reliabilityMonitorV36Installed = true
    task.spawn(function()
...
session.reconnectV36 = session.reconnectV36 or {
    active = false,
    attempts = 0,
    reason = nil,
}
local reconnect = session.reconnectV36
...
local function startReconnect(reason)
    ...
    warn("[XOMA V36] Network disconnect detected | rejoin armed | " .. reconnect.reason)

    task.spawn(function()
        task.wait(1)
        while session.alive and reconnect.active do
            reconnect.attempts = reconnect.attempts + 1
            print("[XOMA V36] Rejoin attempt " .. tostring(reconnect.attempts))
            local ok, err = pcall(function()
                TeleportService:Teleport(game.PlaceId, player)
            end)
            if not ok then
                warn("[XOMA V36] Rejoin request failed: " .. tostring(err))
            end
...
if session.reconnectWatchersV36Installed ~= true then
    session.reconnectWatchersV36Installed = true
...
session.reliabilityBuild = "PASS26-AUTORETRY-RECONNECT-RECORDER-V36"
print("[XOMA V36] Auto Retry forced ON | reconnect installed | recorder -> V36")
return session.XOMA
