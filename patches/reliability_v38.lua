...
-- Build: PASS28-AUTORETRY-RECONNECT-RECORDER-V38
...
then
    error("XOMA V38 reliability patch: CTDIG/XOMA session unavailable")
end
...
local BOOTSTRAP_URL = "https://raw.githubusercontent.com/evilxoma/xomahub/refs/heads/main/ctdui_v38.lua"
...
local function upgradeSavedMacro()
    ...
    if not wrote then
        warn("[XOMA V38] macro bootstrap update failed: " .. tostring(err))
        return false
    end

    print("[XOMA V38] Saved macro bootstrap upgraded -> ctdui_v38.lua")
    return true
end

session.upgradeSavedMacroBootstrapV38 = upgradeSavedMacro
forceAutoRetry()
upgradeSavedMacro()

if session.reliabilityMonitorV38Installed ~= true then
    session.reliabilityMonitorV38Installed = true
    task.spawn(function()
...
session.reconnectV38 = session.reconnectV38 or {
    active = false,
    attempts = 0,
    reason = nil,
}
local reconnect = session.reconnectV38
...
local function startReconnect(reason)
    ...
    warn("[XOMA V38] Network disconnect detected | rejoin armed | " .. reconnect.reason)

    task.spawn(function()
        task.wait(1)
        while session.alive and reconnect.active do
            reconnect.attempts = reconnect.attempts + 1
            print("[XOMA V38] Rejoin attempt " .. tostring(reconnect.attempts))
            local ok, err = pcall(function()
                TeleportService:Teleport(game.PlaceId, player)
            end)
            if not ok then
                warn("[XOMA V38] Rejoin request failed: " .. tostring(err))
            end
...
if session.reconnectWatchersV38Installed ~= true then
    session.reconnectWatchersV38Installed = true
...
session.reliabilityBuild = "PASS28-AUTORETRY-RECONNECT-RECORDER-V38"
print("[XOMA V38] Auto Retry forced ON | reconnect installed | recorder -> V38")
return session.XOMA
