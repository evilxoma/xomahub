...
if HAS_HOOK then
    ...
    oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(self, ...)
        ...
        if shouldRecord and self == UpgradeRemote and method == "InvokeServer" and State.RecordUpgrade then
            ...
            local before = readTowerLevel(tower)
            local seq = reserveActionSeq()
            local result = table.pack(oldNamecall(self, ...))
            task.spawn(function()
                ...
                if id and now ~= nil and (before == nil or now ~= before) then
...
local function invokePlace(name, pos, targetPath)
    ...
    for attempt = 1, 160 do
        ...
        if attempt % 20 == 0 then AddLog("Waiting to place " .. name .. "...") end
        task.wait(0.20)
...
local function upgradeTo(state, targetLevel)
    ...
    local attempts = 0
    while validTower(state.Instance) do
        local current = readTowerLevel(state.Instance)
        if current ~= nil and current >= targetLevel then return true end
        attempts = attempts + 1
        State.InternalCall = true
        pcall(function() UpgradeRemote:InvokeServer(state.Instance) end)
        State.InternalCall = false
        local old = current
        ...
        until not validTower(state.Instance) or current ~= old or os.clock() >= deadline
        if attempts >= 160 then return false end
        if current == old then task.wait(0.20) end
    end
    return false
end
...
function API:Run(strategy)
    ...
    Runtime.Strategy = strategy
    AddLog("START STRATEGY -> " .. tostring(#strategy.Actions) .. " actions / no automatic timestamps")

    local key = "__XOMAHUB_RUN_" .. tostring(game.JobId) .. "_" .. tostring(game.PlaceId)
    if GENV[key] then
        AddLog("Strategy already started in this server")
        return API
    end
    GENV[key] = true
...
MacroTab:Slider({
    ...
    Callback = function(value)
        ...
        saveConfig()
        AddLog(string.format("Place timing -> %.2fs", State.PlaceTiming))
    end,
})
...
MacroTab:Divider()
MacroTab:Button({
    Title = "Execute ctdcool.lua Now",
    Desc = "Load the saved strategy with readfile and execute it.",
    Icon = "play",
    Callback = function()
        if not (HAS_READFILE and HAS_ISFILE and isfile(MACRO_FILE)) then
            WindUI:Notify({ Title = "Macro", Content = MACRO_FILE .. " was not found.", Icon = "triangle-alert", Duration = 3 })
            return
        end
        local ok, err = pcall(function() loadstring(readfile(MACRO_FILE))() end)
        AddLog("EXECUTE SAVED MACRO -> " .. tostring(ok))
...
AddLog("XomaHub loaded")
AddLog("Recorder hook -> " .. tostring(HAS_HOOK))
AddLog("readfile/writefile -> " .. tostring(HAS_READFILE) .. "/" .. tostring(HAS_WRITEFILE))
WindUI:Notify({
    Title = "CTD Macro",
